...
int s[N*10], L, R, n;
bool add[N*10];
inline void pushup(const int &x) { if(add[lc]) s[x]+=s[lc]; if(add[rc]) s[x]+=s[rc]; if(!add[lc] && !add[rc]) s[x]=0; }
inline void pushdown(const int &x, const int &m) {
	if(add[x]) {
		add[lc]^=1; add[rc]^=1;
		if(add[lc]) s[lc]=(m-(m>>1));
		if(add[rc]) s[rc]=m>>1;
		add[x]=0;
	}
}
void update(const int &l, const int &r, const int &x) {
	if(L<=l && r<=R) { add[x]^=1; if(add[x]) s[x]=r-l+1; else s[x]=0; return; }
	pushdown(x, r-l+1);
	int m=MID;
