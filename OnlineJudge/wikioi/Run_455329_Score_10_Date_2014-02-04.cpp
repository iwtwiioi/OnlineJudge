...
void init(int key, int d) {
	if(root == null || who == d) { insert(root, key); who = d; return; }
	insert(root, key);
	tree succ = ps(root, 0), pred = ps(root, 1);
	if(succ == null) { ans += K(pred) - K(root); del(root); del(root); return; }
	if(pred == null) { ans += K(root) - K(succ); del(root); del(root); return; }
	int l = K(root) - K(succ), r = K(pred) - K(root);
	del(root);
	if(l <= r) { ans = (ans + l) % 1000000; splay(succ, root); del(root); }
	else { ans = (ans + r) % 1000000; splay(pred, root); del(root); }
	if(root == null) who = d;
}
