...
{
	st t = state;
	if(t.x1+1<4 && t.m[t.x1+1][t.y1] != O) //第一个空格 向右移
	{
		t.p1 = LEFT; t.p2 = NUL;
	...
	t = state;
	if(t.x2+1<4 && t.m[t.x2+1][t.y2] != O) //第二个空格 向右移
	{
		t.p2 = LEFT; t.p1 = NUL;
	...
	t = state;
	if(t.y1+1<4 && t.m[t.x1][t.y1+1] != O) //第一个空格 向下移
	{
		t.p1 = UP; t.p2 = NUL;
	...
	t = state;
	if(t.y2+1<4 && t.m[t.x2][t.y2+1] != O) //第二个空格 向下移
	{
		t.p2 = UP; t.p1 = NUL;
	...
	t = state;
	if(t.x1 > 0 && t.m[t.x1-1][t.y1] != O) //第一个空格 向左移
	{
		t.p1 = RIGHT; t.p2 = NUL;
		t.x1--; t.m[t.x1+1][t.y1] = t.m[t.x1][t.y1];
		t.m[t.x1][t.y1] = O; t.d++;
	...
	t = state;
	if(t.x2 > 0 && t.m[t.x2-1][t.y2] != O) //第二个空格 向左移
	{
		t.p2 = RIGHT; t.p1 = NUL;
		t.x2--; t.m[t.x2+1][t.y2] = t.m[t.x2][t.y2];
		t.m[t.x2][t.y2] = O; t.d++;
	...
	t = state;
	if(t.y1 > 0 && t.m[t.x1][t.y1-1] != O) //第一个空格 向上移
	{
		t.p1 = DOWN; t.p2 = NUL;
		t.y1--; t.m[t.x1][t.y1+1] = t.m[t.x1][t.y1];
		t.m[t.x1][t.y1] = O; t.d++;
	...
	t = state;
	if(t.y2 > 0 && t.m[t.x2][t.y2-1] != O) //第二个空格 向上移
	{
		t.p2 = DOWN; t.p1 = NUL;
		t.y2--; t.m[t.x2][t.y2+1] = t.m[t.x2][t.y2];
		t.m[t.x2][t.y2] = O; t.d++;
...
int main()
{
	...
	while(++n != 10000000)
	{
        ...
        if(check(s)) break;
		t = s;
		if(t.x1+1<4 && t.p1 != LEFT && t.m[t.x1+1][t.y1] != O) //第一个空格 向右移
		{
			t.p1 = LEFT; t.p2 = NUL;
			t.x1++; t.m[t.x1-1][t.y1] = t.m[t.x1][t.y1];
			t.m[t.x1][t.y1] = O; t.d++;
		...
		t = s;
		if(t.x2+1<4 && t.p2 != LEFT && t.m[t.x2+1][t.y2] != O) //第二个空格 向右移
		{
			t.p2 = LEFT; t.p1 = NUL;
			t.x2++; t.m[t.x2-1][t.y2] = t.m[t.x2][t.y2];
			t.m[t.x2][t.y2] = O; t.d++;
		...
		t = s;
		if(t.y1+1<4 && t.p1 != UP && t.m[t.x1][t.y1+1] != O) //第一个空格 向下移
		{
			t.p1 = UP; t.p2 = NUL;
			t.y1++; t.m[t.x1][t.y1-1] = t.m[t.x1][t.y1];
			t.m[t.x1][t.y1] = O; t.d++;
		...
		t = s;
		if(t.y2+1<4 && t.p2 != UP && t.m[t.x2][t.y2+1] != O) //第二个空格 向下移
		{
			t.p2 = UP; t.p1 = NUL;
			t.y2++; t.m[t.x2][t.y2-1] = t.m[t.x2][t.y2];
			t.m[t.x2][t.y2] = O; t.d++;
		...
		t = s;
		if(t.x1 > 0 && t.p1 != RIGHT && t.m[t.x1-1][t.y1] != O) //第一个空格 向左移
		{
			t.p1 = RIGHT; t.p2 = NUL;
			t.x1--; t.m[t.x1+1][t.y1] = t.m[t.x1][t.y1];
			t.m[t.x1][t.y1] = O; t.d++;
		...
		t = s;
		if(t.x2 > 0 && t.p2 != RIGHT && t.m[t.x2-1][t.y2] != O) //第二个空格 向左移
		{
			t.p2 = RIGHT; t.p1 = NUL;
			t.x2--; t.m[t.x2+1][t.y2] = t.m[t.x2][t.y2];
			t.m[t.x2][t.y2] = O; t.d++;
		...
		t = s;
		if(t.y1 > 0 && t.p1 != DOWN && t.m[t.x1][t.y1-1] != O) //第一个空格 向上移
		{
			t.p1 = DOWN; t.p2 = NUL;
			t.y1--; t.m[t.x1][t.y1+1] = t.m[t.x1][t.y1];
			t.m[t.x1][t.y1] = O; t.d++;
		...
		t = s;
		if(t.y2 > 0 && t.p2 != DOWN && t.m[t.x2][t.y2-1] != O) //第二个空格 向上移
		{
			t.p2 = DOWN; t.p1 = NUL;
			t.y2--; t.m[t.x2][t.y2+1] = t.m[t.x2][t.y2];
			t.m[t.x2][t.y2] = O; t.d++;
